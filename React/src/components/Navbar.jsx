...
const Navbar = ({ changePage }) => {
  ...
  return (
    <nav className="bg-brandWhite shadow-md p-4 flex justify-between lg:justify-start items-center sticky top-0 z-50 gap-6">
      ...
      >
        <svg className="w-8 h-8" viewBox="-0.5 0 25 25" fill="none">
      ...
      {isOpen && (
        <div className="lg:hidden flex flex-col mt-4 space-y-4 pb-2 border-t pt-4">
          <button onClick={() => { changePage("home"); setIsOpen(false); }} className="text-left font-medium">Home</button>
          <button onClick={() => { changePage("ebook"); setIsOpen(false); }} className="text-left font-medium">E-Book</button>
        </div>
      )}
